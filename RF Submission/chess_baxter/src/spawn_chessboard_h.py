...
if __name__ == '__main__':
    ...
    board_setup_main = ['rnbqkbnr', 'pppppppp', '', '', '', '', 'PPPPPPPP', 'RNBQKBNR']
    # board_setup = ['rnbqkbnr', 'pppppppp', '', '', '', '', 'PPPPPPPP', 'RNBQKBNR']
    # board_setup = ['********', '********', '********', '*****N**', '********', '******bn', '', 'K*pP**PR']    
    # board_setup = ['********','', 'k*******', '', '', '', '', '']
    board_setup = ['********', '********', '****R***', '*****N**', '****P**P', '*K**N**Q', '', '*******R']    


    main_piece_positionmap = dict()
    main_piece_names = []
    main_poses = dict()

    # Setup goal states using the main board setup (where each piece needs to be)
    for row, each in enumerate(board_setup_main):
        for col, piece in enumerate(each):
            ...
            # main_piece_positionmap[str(row)+str(col)] = [pose.position.x, pose.position.y, pose.position.z-0.93] #0.93 to compensate Gazebo RViz origin difference
            if piece in list_pieces:
                main_piece_positionmap["%s%d" % (piece, col)] = [pose.position.x, pose.position.y, pose.position.z-0.93]
                main_poses["%s%d" % (piece,col)] = pose
                main_piece_names.append("%s%d" % (piece,col))
    ...
    rospy.set_param('board_setup_main', board_setup_main)
    rospy.set_param('board_setup', board_setup) # Board setup
    rospy.set_param('list_pieces', list_pieces) # List of unique pieces
    rospy.set_param('main_piece_target_position_map', main_piece_positionmap) # 3D positions for each square in the chessboard
    ...
    for piece in piece_names:
        print(piece)
        spawn_chess_pieces = rospy.ServiceProxy('spawn_chess_pieces', GameInfo)
        status = spawn_chess_pieces(piece)

    print("LET'S PLAY CHESS ! :D")
